...
import { Search, Loader2, X, Car, MapPin, Calendar, IndianRupee, AlertCircle, CheckCircle2 } from "lucide-react"
        ...
        <div className="rounded-xl border bg-card p-5 space-y-4">
            <div className="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">Verify any challan by number</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSearch} className="flex gap-2">
                ...
                <Button type="submit" size="sm" disabled={!query.trim() || isLoading} className="gap-1.5">
            ...
            {result && (
                <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            <p className="font-mono text-xs text-muted-foreground">Challan #</p>
                            <p className="font-semibold">{result.challan_number}</p>
                        </div>
                        <Badge className={`text-xs ${PAYMENT_COLORS[result.payment_status] ?? ""}`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="flex items-center gap-1.5 text-muted-foreground">
                            ...
                            <span className="font-mono font-medium text-foreground">{result.vehicle_number}</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-muted-foreground">
                            <Calendar className="h-3.5 w-3.5 shrink-0" />
                            <span>{new Date(result.challan_date).toLocaleDateString("en-IN")}</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-muted-foreground col-span-2">
                            <MapPin className="h-3.5 w-3.5 shrink-0" />
                            <span className="truncate">{result.violation_place}, {result.city}</span>
                        </div>
                    </div>
