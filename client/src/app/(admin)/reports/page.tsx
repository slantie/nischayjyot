...
import { VIOLATION_TYPE_LABELS, GRIEVANCE_CATEGORY_LABELS, GUJARAT_PILOT_CITIES } from "@/lib/constants"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Coins, Car, FileText, BarChart3, TrendingUp } from "lucide-react"
...
import type { Challan, Grievance, ViolationType, GrievanceCategory } from "@/lib/supabase/types"
...
export default async function ReportsPage() {
    ...
    type ChallanRow = Pick<Challan,
        "violation_type" | "payment_status" | "fine_amount" | "city" |
        "vehicle_class" | "offender_gender" | "offender_age_group" |
        "number_plate_type" | "challan_date"
    >
    ...
    // KPI values
    const totalRevenue = challans.filter(c => c.payment_status === "paid").reduce((s, c) => s + Number(c.fine_amount), 0)
    const totalPending = challans.filter(c => c.payment_status === "unpaid").reduce((s, c) => s + Number(c.fine_amount), 0)
    ...
    const paymentDonut = [
        ...
        { name: "Disputed", value: paymentCounts["disputed"] ?? 0, color: "#eab308" },
    ...
    const vehicleDonut = [
        { name: "Two Wheeler", value: vehicleCounts["two_wheeler"] ?? 0, color: "#3b82f6" },
        { name: "Four Wheeler", value: vehicleCounts["four_wheeler"] ?? 0, color: "#a855f7" },
        { name: "Commercial", value: vehicleCounts["commercial"] ?? 0, color: "#f97316" },
    ...
    const npDonut = [
        ...
        { name: "Non-Standard", value: npCounts["non_standard"] ?? 0, color: "#ef4444" },
    ].filter(d => d.value > 0)
    ...
    const grievancesByMonth = grievances.reduce<Record<string, number>>((acc, g) => {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                    { label: "Revenue Collected", value: formatCurrency(totalRevenue), icon: Coins, color: "text-green-600", bg: "bg-green-50 dark:bg-green-950/30" },
                    { label: "Pending Revenue", value: formatCurrency(totalPending), icon: Coins, color: "text-red-600", bg: "bg-red-50 dark:bg-red-950/30" },
                    { label: "Total Challans", value: challans.length.toLocaleString(), icon: Car, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-950/30" },
                    { label: "Total Grievances", value: grievances.length.toLocaleString(), icon: FileText, color: "text-purple-600", bg: "bg-purple-50 dark:bg-purple-950/30" },
                ].map(item => (
                    <Card key={item.label}>
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-base flex items-center gap-2">
                        <TrendingUp className="h-4 w-4 text-primary" /> Monthly Trend — Challans & Grievances
                    </CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <Card>
                        <CardHeader className="pb-0">
                            <CardTitle className="text-sm text-muted-foreground">Vehicle Class</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-2">
                    ...
                    <Card>
                        <CardHeader className="pb-0">
                            <CardTitle className="text-sm text-muted-foreground">Gender Distribution</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-2">
                    ...
                    <Card>
                        <CardHeader className="pb-0">
                            <CardTitle className="text-sm text-muted-foreground">Age Group</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-2">
                    ...
                    <Card>
                        <CardHeader className="pb-0">
                            <CardTitle className="text-sm text-muted-foreground">Number Plate Type</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-2">
                    ...
                    <Card>
                        <CardHeader className="pb-0">
                            <CardTitle className="text-sm text-muted-foreground">Grievance Categories</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-2">
